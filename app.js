...
// DOM Elements for Jobs
const jobsTabsContainer = document.getElementById("jobs-tabs-container");

function saveAppState() {
  const activeJob = jobs.find(j => j.id === currentJobId);
  if (activeJob) {
    activeJob.criteria = criteria;
    activeJob.historyBatches = historyBatches;
    if (currentBatchId === "active") {
      activeJob.candidates = candidates;
    }
  }
  localStorage.setItem("recruitsync_jobs", JSON.stringify(jobs));
  localStorage.setItem("recruitsync_current_job_id", currentJobId);
}
...
function initAppState() {
  const storedJobs = localStorage.getItem("recruitsync_jobs");
  const storedCurrentJobId = localStorage.getItem("recruitsync_current_job_id");
  ...
  if (storedJobs) {
    ...
    const oldCriteria = storedCriteria ? JSON.parse(storedCriteria) : [...defaultCriteria];
    const oldCandidates = storedCandidates ? JSON.parse(storedCandidates) : [...candidatesData];
    ...
    jobs = [
      {
        ...
        historyBatches: oldHistory
      }
    ];
    localStorage.setItem("recruitsync_jobs", JSON.stringify(jobs));
  }

  if (storedCurrentJobId && jobs.find(j => j.id === storedCurrentJobId)) {
    currentJobId = storedCurrentJobId;
  } else {
  ...
  const activeJob = jobs.find(j => j.id === currentJobId);
  criteria = activeJob.criteria || [];
  historyBatches = activeJob.historyBatches || [];
...
function renderCandidates() {
  ...
  if (currentBatchId === "active") {
    ...
    btnArchiveBatchEl.disabled = candidates.length === 0;
  } else {
...
btnResetApp.addEventListener("click", () => {
  if (confirm("Are you sure you want to reset all candidate profiles, criteria, and batch history to default? Your uploaded resumes, custom criteria, and archived batches will be deleted.")) {
    localStorage.removeItem("recruitsync_jobs");
    localStorage.removeItem("recruitsync_current_job_id");
...
function renderHistoryList() {
  ...
  const activeJob = jobs.find(j => j.id === currentJobId);
  const activeCandidatesCount = activeJob ? activeJob.candidates.length : 0;
...
function confirmAndArchiveBatch() {
  ...
  const activeJob = jobs.find(j => j.id === currentJobId);
  const activeCandidates = activeJob ? activeJob.candidates : [];
...
function exportAppState() {
  ...
  a.href = url;
  a.download = `recruitsync_jobs_backup_${timestamp}.json`;
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  URL.revokeObjectURL(url);
  
  showStatus("All jobs and history exported successfully!", "success");
}
...
function importAppState(event) {
  ...
  reader.onload = function(e) {
    try {
      ...
      let newJobs = null;
      let newCurrentJobId = null;

      if (importedData.jobs && Array.isArray(importedData.jobs)) {
        newJobs = importedData.jobs;
        newCurrentJobId = importedData.currentJobId || newJobs[0].id;
      } else if (importedData.criteria || importedData.candidates || importedData.historyBatches) {
        newJobs = [
          {
            id: "job_migrated_" + Date.now(),
            title: "Imported Role",
            criteria: importedData.criteria || [],
            candidates: importedData.candidates || [],
            historyBatches: importedData.historyBatches || []
          }
        ];
        newCurrentJobId = newJobs[0].id;
      } else {
        throw new Error("Invalid backup file format.");
      }

      if (confirm("Importing this backup will overwrite all current job roles, criteria, candidates, and batch history. Are you sure you want to proceed?")) {
        localStorage.setItem("recruitsync_jobs", JSON.stringify(newJobs));
        localStorage.setItem("recruitsync_current_job_id", newCurrentJobId);
...
// Job Role Tabs Operations
function renderJobTabs() {
  if (!jobsTabsContainer) return;
  jobsTabsContainer.innerHTML = "";

  jobs.forEach(job => {
    const tab = document.createElement("div");
    tab.className = `job-tab ${currentJobId === job.id ? 'active' : ''}`;
    tab.onclick = () => switchJob(job.id);

    tab.innerHTML = `
      <span>${job.title}</span>
      ${jobs.length > 1 ? `
        <button class="btn-delete-job" title="Delete Role" onclick="event.stopPropagation(); deleteJobRole('${job.id}')">×</button>
      ` : ''}
    `;
    jobsTabsContainer.appendChild(tab);
  });
}
...
function addJobRolePrompt() {
  const roleName = prompt("Enter the title for the new Job Role (e.g. React Developer, Account Manager):");
  ...
  historyBatches = newJob.historyBatches;
  currentBatchId = "active";
...
function deleteJobRole(jobId) {
  const jobToDelete = jobs.find(j => j.id === jobId);
  if (!jobToDelete) return;

  if (confirm(`Are you sure you want to delete the job role "${jobToDelete.title}"? All associated candidate resumes and history batches will be permanently lost.`)) {
